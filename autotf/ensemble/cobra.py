...
class Cobra(BaseEstimator):
    ...
    def opmimal_parameters(self, X = None, y=None, eps_size=None,  grid_points=None):
        ...
        self.set_alpha(X_epsilon=X_eps, y_epsilon=y_eps)
    ...
    def set_alpha(self, X_epsilon=None, y_epsilon=None):
    ...
    def set_split(self, X_epsilon=None, y_epsilon=None):
        ...
        for k, l in split:
            if self.regression:
                machine = Cobra(X=X_epsilon, y=y_epsilon, epsilon=self.epsilon, models=self.models,
                ...
                machine = Cobra(X=X_epsilon, y=y_epsilon, epsilon=self.epsilon, models=self.models,
                                ...
                                alpha=self.alpha)
            machine.split_data(int(k * len(self.X_)), int((k + l) * len(self.X_)))
            machine.load_machines()
            machine.load_machine_predictions()
            results = machine.predict(X_epsilon)
            Score[(k, l)] = (self.metric(y_epsilon, results))
        if self.regression:
