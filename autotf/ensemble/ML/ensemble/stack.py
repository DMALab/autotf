...
class Stacking():
    ...
    def add(self, models, propagate_features=None, subset=None):
        ...
        for model_counter, model in enumerate(models):
            ...
            if self.X_test is not None and self.bagged_pred == True:
            ...
            list_index = []
            for tr_index, te_index in kf.split(self.next_train, self.y_train):
                list_index.append((tr_index, te_index))
            for fold_counter, index in enumerate(list_index):
                tr_index = index[0]
                te_index = index[1]
                ...
                if self.X_test is not None and self.bagged_pred:
                    if 'predict_proba' == action:
    ...
    def add_meta(self, meta_model, needs_proba = None):
        if needs_proba is None:
            self.needs_proba = None
