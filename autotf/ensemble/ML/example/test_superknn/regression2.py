...
models = [linear_model.LassoCV(random_state=0, n_jobs=10), GradientBoostingRegressor(random_state=0),
          SVR(),RandomForestRegressor(random_state=0, n_jobs=10,
                          n_estimators=100, max_depth=3),
          ExtraTreesRegressor(random_state=0, n_estimators=100, n_jobs=10),
          XGBRegressor(random_state=0, n_estimators=100, n_jobs=10)
          ]
# XGBRegressor(random_state=0, n_estimators=100, n_jobs=2)
# # '''With optimal'''
model = Superknn(models=models, epsilon=3.65,alpha=6, regression=True,
                 metric=mean_absolute_error, random_state=0, folds=5, n_jobs=20)
# model.pre()
...
# model.opmimal_parameters(X=X_train, y=y_train, eps_size=0.2, grid_points=10)
...
# epsilons = np.arange(3.65, 3.67, 0.01)
