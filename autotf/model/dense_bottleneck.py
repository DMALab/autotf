...
class dense_bottleneck(BaseModel):
    default_param = {
        "loss": "square_loss",
        "metrics": ["loss"],
        "optimizer": "sgd",
    ...
    def build_model(self):
        ...
        #self.conv1 = self.or(self.inputs, 16, 3, regularizer='L2', weight_decay=0.0001)
        self.net = self.densenet_bottleneck("densenet1",self.conv1, self.block_num, self.growth)

        self.net = self.densenet_bottleneck("densenet2", self.net, self.block_num, self.growth)

        self.net = self.densenet_bottleneck("densenet3", self.net, self.block_num, self.growth)
    ...
    def densenet_bottleneck(self,layer_name,inputs, nb_blocks, growth, downsample=True, downsample_strides=2):
        densenet = inputs
        ...
        with tf.variable_scope(layer_name) as scope:
            for i in range(0, nb_blocks):
                ...
                densenet = self.bn("bna"+str(i),densenet)
                densenet = tf.nn.relu(densenet)
                densenet = self.origin_conv2d(str(i)+"conv2da",densenet,growth,1,1)

                densenet = self.bn("bnb" + str(i), densenet)
                densenet = tf.nn.relu(densenet)
                densenet = self.origin_conv2d(str(i) + "conv2db", densenet, growth, 3, 1)
                densenet = tf.concat([densenet, conn], 3)
